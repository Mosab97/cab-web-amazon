...
use Alone\LaravelHuaweiPush\{HuaweiNotification ,HuaweiMessage};
...
function setting($attr)
{
  if (\Schema::hasTable('settings')) {
    ...
    if (in_array($attr, ['logo','fcm_notification_sound','fcm_notification_order_sound'])) {
        if ($attr == 'logo') {
            return ! empty($setting) ? asset('storage/images/setting')."/".$setting->value : asset('dashboardAssets/images/cover/cover_sm.png');
        }else{
            return ! empty($setting) ? asset('storage/files/setting')."/".$setting->value : 'default';
        }
    }
    if ($phone == 'phone') {
...
function uploadFile($files, $url = 'files', $key = 'file', $model = null)
{
  ...
  if (gettype($files) == 'array') {
    ...
    foreach ($files as $new_file) {
        $file_name = time() . "___file_" . str_replace(" ",'_',$new_file->getClientOriginalName());
    ...
    $file = $files;
    $file_name = time() . "___file_" . str_replace(" ",'_',$file->getClientOriginalName());
...
function pushFcmNotes($fcmData, $userIds,$model = '\\App\\Models\\Device')
{
  ...
  if (is_array($userIds) && !empty($userIds)) {
    ...
    $huawei_devices = array_filter($devices->where('type','huawei')->pluck('user_id')->toArray());
    ...
    ->setSound(@$fcmData['fcm_sound'] ?? 'default');
    ...
    if (count($ios_devices)) {
     ...
     if (count($huawei_devices)) {
         $users = User::whereIn('id',$huawei_devices)->get();
         $msg = (new HuaweiNotification)
          ...
          $send_process = count($huawei_devices);
     }
...
function weekOfMonth($weekOfYear,$date){
    $firstOfMonth = date("Y-m-01",strtotime($date));
    return $weekOfYear - weekOfYear($firstOfMonth) + 1;
}
