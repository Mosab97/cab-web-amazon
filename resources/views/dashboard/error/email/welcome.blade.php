    ...
    <body dir="rtl" style="text-align: center;font-family: 'Cairo', sans-serif;background-color: #f5f5f5;padding: 20px;font-size: 14px; line-height: 1.43;">
        <div style="max-width: 600px; margin: 0px auto; background-color: #fff; box-shadow: 0px 20px 50px rgba(0,0,0,0.05);">
            ...
            <div style="padding: 30px 70px; border-top: 1px solid rgba(0,0,0,0.05);">
                ...
                <h4>رسالة تفعيل الحساب لتطبيق {{ setting('project_name') }}</h4>
                <div style="color: #636363; font-size: 14px;">
                    <p>
                    للدخول الى لوحة التحكم استخدم بريدك الالكترونى
                     {{ $notifiable->email }}
                     وكتابة كلمة مرور جديدة من خلال الرابط
                    </p>
                </div>
                <div style="position: relative;margin: 20px 0;">
                        @if ($notifiable->roles()->exists())
                            <h3>رابط اللوحة </h3>
                            <h3 style="border: 1px; color: #0091d7">
                                <a href="{{ route('confirmation_path', $notifiable->verified_code) }}">اضغط هنا</a></h3>
                        @else
                            <h3>كود التفعيل</h3>
                            <h3 style="border: 1px; color: #0091d7">{{ $notifiable->verified_code }}</h3>
                        @endif
                </div>
            </div>
            <div style="background-color: #00e573; padding: 40px; text-align: center;">
                <div style="color: #fff; font-size: 10px;">جميع حقوق الملكية محفوظة لتطبيق {{ setting('project_name') }}
                </div>
            </div>
        </div>
    </body>
