                    ...
                    <div class="mb-4">
                        ...
                        <ul class="list-inline list-inline-dotted text-muted mb-3">
                            <li class="list-inline-item">
                                {{ trans('dashboard.contact.writtenBy') }} <a href="{{ $contact->user_id ? route('dashboard.'.$contact->user->user_type.'.show',$contact->user_id) : '#' }}" class="text-muted"> {{ $contact->fullname }}</a>
                            </li>
                            <li class="list-inline-item">
                            ...
                            @if ($contact->order_id)
                            <li class="list-inline-item">
                                <a href="{{ route('dashboard.order.show',$contact->order_id) }}">{{ $contact->order_id }}</a>
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        <label class="control-label col-lg-2">
                            {{ trans('dashboard.setting.sending_type') }}</label>
                                @if($contact->user)
            ...
            <div class="sidebar-content">
                ...
                <div class="card border-info bg-transparent">
                    <div class="card-header">
                        <h4 class="card-title">{!! trans('dashboard.contact.latest_messages') !!}</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <ul class="activity-timeline timeline-left list-unstyled" style="padding-right:30px;">
                                @forelse ($recentMsgs as $recent)
                                <li>
                                    <div class="timeline-icon avatar">
                                        <a href="{{ route('dashboard.contact.show',$recent->id) }}" class="media-title">
                                            <img src="{{ $recent->image }}" class="rounded-circle" width="30" height="30" alt="">
                                        </a>
                                    </div>
                                    <div class="timeline-info">
                                        <a href="{{ route('dashboard.contact.show',$recent->id) }}" class="media-title">
                                            <p class="font-weight-bold">{{ $recent->fullname }}</p>
                                        </a>
                                        <span>{{ str_limit($recent->content , 50 , "...") }}</span>
                                    </div>
                                    <small class="">{{ $recent->created_at->format('d M,Y') }}</small>
                                </li>
                                @empty
